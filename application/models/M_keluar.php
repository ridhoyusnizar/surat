...
class m_keluar extends CI_Model
{
    ...
    {
        return $this->db->get_where($this->_table, ["id_surat_keluar" => $id])->row();
    }
    ...
    {
        $config['upload_path']          = './upload/suratkeluar/files/';
    ...
    private function _deleteImage($id)
    {
        ...
        if ($surat_keluar->gambar != "default.jpg") {
            $filename = explode(".", $surat_keluar->gambar)[0];
            return array_map('unlink', glob(FCPATH . "upload/suratkeluar/files/$filename.*"));
        }
    }
